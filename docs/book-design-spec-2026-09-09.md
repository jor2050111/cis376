...
| 2 | copperwind_ops: measure table sizes and monthly ticket growth, project storage 36 months out | A full | `ERROR: function pg_size_pretty(numeric) does not exist` (a `sum()` returned numeric, cast to bigint fixes it) | Ch 1 | the growth projection query, the pg_hba.conf rule order walkthrough |
